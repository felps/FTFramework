...
public abstract class Ftec extends java.rmi.server.UnicastRemoteObject
	...
	@Override
	public void run() {
		Process runningApp = this.startExecution(this.getAppBinaryLocation(),
				this.getAppDescriptorFileLocation());
	...
	public abstract Long startFtecThread(String configFile, Task submittedTask,
			int uid);

	protected Process startExecution(String appBinaryType,
			String configFileLocation) {
		// DEBUG
		System.out.println("Esta é a " + getTimesRan()
				+ "-ésima vez que dah um startExecution");

		Process runningApp = null;

		try {
			runningApp = Runtime.getRuntime().exec(
					getIGHOME() + "/startservices.sh asctText -i "
							+ configFileLocation + " -b " + appBinaryType);
		} catch (IOException e) {
			System.out.println("Erro na submissão da tarefa "
					+ submitedTask.getName());
			System.out.println("Comando: " + getIGHOME()
					+ "/startservices.sh asctText -i " + configFileLocation
					+ " -b " + appBinaryType);
			e.printStackTrace();
		}
		System.out.println(getIGHOME() + "/startservices.sh asctText -i "
				+ configFileLocation + " -b " + appBinaryType);
		return runningApp;
	}
	...
	protected void waitCompletion(Process runningApp) {
		try {
			...
			InputStream stderr = runningApp.getErrorStream();
			InputStream stdout = runningApp.getInputStream();
			String line;
			...
			try {
				while ((line = reader.readLine()) != null) {
					...
					MyLogger.getLogger().info("STDOUT: "+ line);
				}
			} catch (IOException e) {
			}
			...
			try {
				while ((line = reader.readLine()) != null) {
					...
					MyLogger.getLogger().info("STDERR: "+ line);
				}
			} catch (IOException e) {
			}
	...
	protected void endExecution(Process runningApp) {
		...
		try {
			this.reportFinishedFtec(this.getFtecId(), this.getTask().getName(),
	...
	protected synchronized void reportFinishedFtec(int ftecId, String taskName,
			String workflowName) throws RemoteException, InterruptedException {
	...
	protected static void reportFinishedFtecToFTM(int ftecId)
	...
	public static int requestFtecThread(String configFile, Task submittedTask,
			String chosenFtecService, int uid) {
	...
	public static FtecInterface getFtecObject(String ftecServiceName) {
		...
		try {
			ftec = (FtecInterface) Naming.lookup("rmi://localhost:1099/"
					+ ftecServiceName);
	...
	private static TaskSubmission getFtmObject() {
		...
		try {
			ftm = (TaskSubmission) Naming
					.lookup("rmi://localhost:1099/FTMService");
		} catch (MalformedURLException murle) {
			System.out.println();
			System.out.println("MalformedURLException");
			System.out.println(murle);
